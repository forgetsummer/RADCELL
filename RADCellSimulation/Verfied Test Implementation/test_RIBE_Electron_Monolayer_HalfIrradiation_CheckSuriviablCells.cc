...
//         mySim.SetCellWorld(0.1,0.1,0.1);//create cell world, dimension in unit of mm, this line is for testing single cell simulation
    ...
    std::map<int, double> cellPositionXX;
    std::map<int, double> cellPositionYY;
    std::map<int, double> cellPositionZZ;
    
    
    for (int i=0; i<layout.GetCellNumber(); i++)
    {
        cellPositionXX[i] = layout.GetCellPositionX(i);
        cellPositionYY[i] = layout.GetCellPositionY(i);
        cellPositionZZ[i] = layout.GetCellPositionZ(i);
    ...
    testSource.RadiationPlaneSourceSetUp(testRadiationName,particleNumber,particleEnergy, xDim/4,0,0.4, xDim/2,yDim);
...
//*****************************************************************************************************************
    ...
    for (int n =14;n<15;n++)// simulation for different prescribed dose
...
//                     cout<<"the concentration is "<<concentration<<endl;
                    double concentrationInMass;
//                     if (cX>=0 && cX<xDim/2)
//                     {
//                         concentrationInMass  = 0 ; // when the cell staying at the spared region, and with the wall, we do not consider bystander signals migration
//                     }
//                     else
//                     {
...
//                     }
                    ...
                    if (cycle_cellStateUPdate==1)
                    {
                        ...
                        double DSB_cell = radiationTransResulut.GetAbsDSBOfCell(mitr_cell->first);// get absolute DSB of cell
                        double DSB_std_cell = radiationTransResulut.GetAbsDSBStdOfCell(mitr_cell->first);//get absolute DSB std of cell
//                         DSB_cell = DSB_cell+ DSB_std_cell;// the upper bound of DSB of cell
                        DSB_cell = DSB_cell-DSB_std_cell; // the lower bound of DSB of cell
...
//********************************************************************************************************
        ...
        for (int i =0;i<cellNum;i++)
        {
            cellColonySizeMap[i] = 0; // initialize the cell colony map
        }
...
//         cellSF_private.push_back((double)colonyNumber/cellNum);// store cell sf to cellSurivalFractionVec
        ...
        mySim.CreateCell("SurvivalCell","Cytoplasma Nucleus","Sphere","Blue Green");
        mySim.CreateCell("DeadCell","Cytoplasma Nucleus","Sphere","Red Green");
        mySim.SetCellSimulationParameter(1," Nucleus");
        
//      
        cout<<"The size of cellColonySizeMap is "<<cellColonySizeMap.size()<<endl;
        for (std::map<int, int >::iterator mitr_cell = cellColonySizeMap.begin();mitr_cell!=cellColonySizeMap.end();mitr_cell++)
        {
            cout<<"The colonyNumber for cell "<<mitr_cell->first<< " is "<<mitr_cell->second<<endl;
            if (mitr_cell->second>=colonySizeThreshold)// if cell is survival
            {
                mySim.CellConstruction(mitr_cell->first,"SurvivalCell",cellPositionXX.at(mitr_cell->first),\
                 cellPositionYY.at(mitr_cell->first),cellPositionZZ.at(mitr_cell->first),20,5); // seed the survival cells
            }
            else // if cell is dead
            {
                mySim.CellConstruction(mitr_cell->first,"DeadCell",cellPositionXX.at(mitr_cell->first),\
                 cellPositionYY.at(mitr_cell->first),cellPositionZZ.at(mitr_cell->first),20,5);// seed the dead cells
            }
        }
