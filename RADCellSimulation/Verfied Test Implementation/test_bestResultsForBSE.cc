...
struct cellSFObject
{
    ...
    int colonyNumber;
    double sf;
    std::map<int, double>  positionX;
    std::map<int, double>  positionY;
    std::map<int, double>  positionZ;
...
//*************************************************************************************
    ...
    for (int i =0;i<=5;i++) // fine dose step for dose lower than 1Gy
    {
        prescribedDoseVec.push_back(0.2*i);
    }
    for (int i=3;i<=20;i++)
    {
...
//         mySim.SetCellWorld(0.1,0.1,0.1);//create cell world, dimension in unit of mm, this line is for testing single cell simulation
    ...
    testSource.RadiationPlaneSourceSetUp(testRadiationName,particleNumber,particleEnergy,xDim,yDim);
...
//     ********************** Single Cell Irradiation for calculate the absolute dose and DNA damage of cells
    ...
    singleCellIrradiationSource.RadiationPointSourceSetUp("Mono_Electron",5000,particleEnergy);
...
//*****************************************************************************************************************
    std::vector<std::vector<cellSFObject> >globalCellSFInfo; // a vector storing all the cell sf information for all the history
    std::vector<string> globalCellSFRunHistoryID;
    int totalHisoryNum = 5;
    for (int k=0;k<totalHisoryNum;k++) // run 10 times for estimating the statistic erros of cell surival fractions
    {

        string cellPhaseFileName;
        string cellStateFileName;
        stringstream ssRunID;
        ssRunID<<k;
        string runFileID = ssRunID.str();
        cellPhaseFileName = "run_"+runFileID+"_"+ testSource.GetSimulationSourceID()+"_cellPhase.csv";
        cellStateFileName = "run_"+runFileID+"_"+ testSource.GetSimulationSourceID()+"_cellState.csv";
        ofstream file;
        ofstream file1;
...
//         for (int n =0;n<1;n++)// simulation for different prescribed dose
    ...
    //                     double concentrationInMass = 1E+4*massOfBystanderSignal/volume;
                        if (cycle_cellStateUPdate==1)
                        {
                            ...
                            double DSB_cell_std = radiationTransResulut.GetAbsDSBStdOfCell(mitr_cell->first);
                            
                            //***********************************
                            double sampled_DSB_cell;
                            bool select_true = false;
                            int kkk=0;
                            while (select_true == false)
                            {
                                double Xf;
                                double pi = 3.1415926535897;
                                sampled_DSB_cell = DSB_cell+ DSB_cell_std*sqrt(-2*log(G4UniformRand()))*cos(2.0*pi*G4UniformRand()); // Gaussian sampling the DSB of cell
                                if (sampled_DSB_cell>= DSB_cell-DSB_cell_std && sampled_DSB_cell<=DSB_cell+DSB_cell_std && sampled_DSB_cell>=0)// if within the interval
                                {
                                    select_true = true;
                                }
                            }
                            ...
                            myCellState.CellStateUpdate(mitr_cell->first,cellType,sampled_DSB_cell,0,deltaT_cellStateUpdate,1);
                        }
                        else
                        {
    //                         cout<<"concentrationInMass is "<<concentrationInMass<<endl;
            ...
            runSFRes.positionX = cellPositionX;
            runSFRes.positionY = cellPositionY;
            runSFRes.positionZ = cellPositionZ;
            
            cellSF_private[n] = runSFRes; // store the cell surival fraction information into cellSFObject
            cout<<"get here end"<<endl;
        ...
        globalCellSFInfo.push_back(cellSF_private);
        globalCellSFRunHistoryID.push_back(cellStateFileName);
                   
    }
    
    //*** finish all the run, then start to do the statistical error analysis
    
    std::vector<std::vector<double> > sfVec;
    
    for (int i=0;i<globalCellSFInfo.size();i++)
    {
        sfVec.push_back(std::vector<double>());
        for (int j=0;j<globalCellSFInfo[i].size();j++)
        {
            sfVec[i].push_back(globalCellSFInfo[i][j].sf);
        }
    }
    
    MonteCarloErrorAnalysis testErrorAnalysis; 
    testErrorAnalysis.ImportSimulationResults(sfVec);
    std::vector<double> meanSfVec = testErrorAnalysis.GetMeanOfSimulationResults();
    std::vector<double> stdSfVec  = testErrorAnalysis.GetStdOfSimulatioinResults();

    string cellStateFileName_global;
    cellStateFileName_global =testSource.GetSimulationSourceID()+"_cellState.csv";
    ofstream file_g;
    file_g.open(cellStateFileName_global.c_str());
    
    
    for (int i=0;i<meanSfVec.size();i++)
    {
        file_g<<prescribedDoseVec[i]<<","<< meanSfVec[i]<<","<<stdSfVec[i]<<endl;
    }
    
    int historyNum = 0;// check the run information for first history run
    DataProcessUsingPython testProcess;
    string cellStateFileName = globalCellSFRunHistoryID[historyNum];
    testProcess.PlotCellSurvivalCurve(cellStateFileName,"b");//check the cell surival fraction curve of one specific run
    ...
    int doseNum = 1;  
    std::map<int, double> cellPositionX = globalCellSFInfo[historyNum][doseNum].positionX;
    std::map<int, double> cellPositionY = globalCellSFInfo[historyNum][doseNum].positionY;
    std::map<int, double> cellPositionZ = globalCellSFInfo[historyNum][doseNum].positionZ;
    ...
    //     mySim.EnergyDistributionCalculation("gui","microdosimetry.in",true); // check tissue geometry
